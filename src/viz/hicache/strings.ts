...
export const LOCATION = {
  ...
  instance: { en: "Serving instance", zh: "服务实例" },
  ...
  localLink: { en: "CPU ↔ GPU interconnect", zh: "CPU ↔ GPU 互连" },
  ...
  nodeDetail: { en: "GPUs and CPU DRAM are in the same server. This example places one instance on each node; instances can also use other GPU and node layouts.", zh: "GPU 和 CPU DRAM 都在这台服务器内。示例中每个节点运行一个实例；实际实例也可以采用其他 GPU 和节点布局。" },
