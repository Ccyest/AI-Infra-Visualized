...
const COPY = {
  zh: {
    ...
    attnFormula: "hℓ = Σᵢ αℓᵢ bᵢ",
    attnNote: "每层各有一个 pseudo-query，学习该从 Emb / B₁ / B₂ / … 各取多少",
    attnBenefit: "深层对多个旧摘要做 softmax 加权混合，不是精确选择某一层；信息和梯度因此获得更短路径。",
    bank: "旧 block 摘要分别保留",
    target: "送入 B₈ 的第 1 层",
    blockNote: "B₁–B₇ 各 12 层，B₈ 是 L85–93 的 9 层尾段。取回是逐层做的：块内第 2 层起，候选集里还多一份本块到上一层的部分和——那就是块内的普通 residual。",
    why: "每一层都能按内容取回早期 block 的表示，不必全部挤进一条累计流；每 12 层只存一份摘要，把候选集从 93 份压到 9 份来控制开销。",
  ...
  en: {
    ...
    attnTitle: "Attention Residual",
    attnFormula: "hℓ = Σᵢ αℓᵢ bᵢ",
    attnNote: "Each layer has its own pseudo-query deciding how much to retrieve from Emb / B₁ / B₂ / …",
    attnBenefit: "A deep layer forms a softmax-weighted mixture of saved summaries rather than selecting one exact layer, creating shorter paths for information and gradients.",
    bank: "Prior block summaries remain separate",
    target: "input to B₈'s first layer",
    blockNote: "B₁–B₇ hold 12 layers each; B₈ is the 9-layer L85–93 tail. Retrieval happens per layer: from the block's second layer onward the candidate set also holds the block's partial sum so far — that is the ordinary residual inside the block.",
    why: "Every layer can retrieve earlier representations by content instead of squeezing everything into one accumulated stream; one summary per 12 layers shrinks the candidate set from 93 to 9 and keeps the cost down.",
