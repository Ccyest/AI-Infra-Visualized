...
description: "Radix Cache is a major component in SGLang for cross-request cache reuse. Nowadays models apply hybrid attention mechanisms, but the traditional Radix Cache is only designed for full attention KV. Therefore we designed SGLang's Unified Radix Cache for hybrid attention mechanisms. Multi-turn hit rates stay near **98%** with a Mooncake L3 tier. The session-aware configuration lowers SWE-bench TTFT by up to **16.6%**. The Rust tree core lowers SWA TTFT by up to **42%**."
order: 5
concepts: []
