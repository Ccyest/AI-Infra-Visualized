...
| `order` | number | 课程顺序,决定首页排序与上一课/下一课导航 |
...
- 每课以一段「**一句话**」引用块开头,以「图 ↔ 真实系统对照」表结尾,
  诚实标注可视化做了哪些简化;
- **不写「下一课预告」**:课程之间靠自动生成的上一课/下一课导航衔接,
  正文与参考文献标签都不预告未写的课程;
...
- **界面文案集中在本课的 `strings.ts`**,全部提供 zh/en 两份(类型
...
├── viz/<slug>/             # 每课专属的可视化组件、模拟引擎与 strings.ts
